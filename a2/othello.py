...
def minimax_value(board, white_turn, search_depth, alpha, beta):
    ...
    elif not generate_legal_moves(board,white_turn):
        return minimax_value(board, not white_turn, search_depth, alpha, beta)
    else:
        if white_turn:
            for move in generate_legal_moves(board,white_turn):
                new_board = play_move(board,move,white_turn)
                alpha = max(alpha,minimax_value(new_board,False, search_depth - 1, alpha,beta))
                if alpha >= beta:
                    break
            return alpha
        else:
            for move in generate_legal_moves(board,white_turn):
                new_board = play_move(board,move,white_turn)
                beta = min(beta,minimax_value(new_board, True, search_depth - 1, alpha,beta))
                if beta <= alpha:
                    break
            return beta
    return 0
