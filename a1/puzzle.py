...
class NumberPuzzle:
    ...
    def solve(self, better_h):
        ...
        self.key = self.dist_from_start + self.heuristic(BETTER)
        explored = set()
        path = []
        frontier.put(self)
        ...
        while frontier:
            node = frontier.get()
            ##check if it is solved and return the reverse list to show the progress
            if node.solved():
                while node.parent:
            ...
            ##explore the legal moves
            for move in node.legal_moves():
                if  move not in explored:
                    move.key = move.dist_from_start + move.heuristic(BETTER)
                    frontier.put(move)
    ...
    def manhattan_heuristic(self):
        ...
        for i in range(PUZZLE_WIDTH):
            for j in range(PUZZLE_WIDTH):
                ...
                if (standard[i][j] != current) and (current != BLANK):
                    for k in range(PUZZLE_WIDTH):
                        for l in range(PUZZLE_WIDTH):
                            if standard[k][l] == current:
                                total_manhattan += abs(i - k) + abs(j - l)
